...
n.samples <- length(full_norm_svd$d)

(full_norm_svd$d[1]^2)/sum(full_norm_svd$d^2) ## squared 0.9926
(full_norm_svd$d[2]^2)/sum(full_norm_svd$d^2) ## squared 0.0069
...
initial_dat <- data.frame(IDS=group_labels, Plate=plate, Lines=cell_lines, PC1=full_norm_svd$v[, 1], PC2=full_norm_svd$v[, 2], stringsAsFactors=FALSE)

## extract the treatment names from the covariates file. 
treatments_full <- c(sapply(plates, function(this_plate){
    #this_plate <- plates[1]
    barcodes <- as.integer(gsub(paste(this_plate, '-HT', sep=''), '', initial_dat[initial_dat$Plate==this_plate, 'IDS']))
